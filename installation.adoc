...
disable Wayland, we recommend that initially leave it enabled. Whether
your choice works or not should be evident when you start the
<<Second stage installation>> step below. If it doesn't work, the
*NOTE* following the instructions for disabling Wayland covers some
possible recovery methods.

TIP: This step can be executed when the installation stops for input
in the next step, <<Remove installation media>>.
...
If Wayland is enabled when it shouldn't be or not enabled when it
should be, the console may be very difficult, even impossible, to work
with. If you find you have made the wrong choice, there are at least
three possible ways to recover:
...
. Reinstall from scratch and make the opposite choice for this step.
...
=== Suspend and power button issues
...
<<_fsadapt>> in the <<Third stage installation>>. If you chose not to
disable suspend, you may encounter this issue. An easy to way to fix
it is to switch to a different text console and then back again. The
cursor should reappear.

. Disable power switch from shutting the system down
...
. Disable user controls, such as kbd:[Ctlr+Alt+Del] to reboot

+

This item is provided without a detailed explanationI because it is
not expected that anyone will need it. If you do, the file
link:powerlock.tar.gz[] may be helpful. It contains the contents of
the files that need to be changed or created.
