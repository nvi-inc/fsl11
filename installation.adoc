...
* 1.1.4 -- Correctly set non-_root_ user _~/.ssh_ ownership
...
You will need to create the _~oper/.ssh_ directory if it doesn't already exist with:
...
If created the _~oper/.ssh_ directory, set its ownership (analogously
for _prog_) with:
