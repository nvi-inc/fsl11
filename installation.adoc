...
* 1.2.1 -- Add security update cookbook; Add how to overcome not checking
weekly cron output for kernel updates
...
The first section, <<Cookbook for installing updates (upgrading)>>, of
this appendix provides a minimal set of instructions for installing
security updates. The remaining sections provide a more comprehensive
treatment of managing these updates.
...
document. This will keep you informed of the available updates on a
weekly basis.
...
=== Cookbook for installing updates (upgrading)
...
All steps are to be preformed by _root_:
...
below). Before the instructions, there may be `NEWS` items. These
rarely occur, but may contain additional actions that are needed. If
there are any, be sure you are prepared to perform them before
...
<<Removing insecure packages>> section below). Those can removed
...
handling a kernel update if one is available. If you don't use the
weekly job, or don't check its output, you should check for updates
manually (see the <<Manually checking for updates>> section below) to
see if there are instruction about kernel updates, before running
`*apt{nbsp}upgrade*` (below). See the <<Kernel updates>> section below
for additional considerations for kernel updates.

The commands for installing the updates given by the message are (note
        the use of _apt_ instead of _apt-get_):

CAUTION: If the `apt upgrade` command below says it will install `NEW`
packages, it is recommended to understand why they are being installed
before confirming. Usually new packages will be related to a kernel
update (see the <<Kernel updates>> section below) or related to an
insecure package that was removed (see the
<<Removing insecure packages>> section below). Normally, any
reinstalled insecure packages should be removed again after upgrading.
...
<<Installing updates (upgrading)>> section above, except you will use
the instructions from the output of the script above instead of from
the weekly _cron_ job (the outputs should be equivalent for the same
set of updates). Additionally, please read the following *NOTE*.
