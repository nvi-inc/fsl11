...
* 1.2.1 -- Add how to overcome not checking weekly cron output for
kernel updates
...
This section gives "`cookbook`" instructions for upgradng. It collects
best practices from elsewhere in the FSL11 documents. Links to
...
This may take a few minutes. If there is no output, there are no
updates to install.
...
there are any, be sure you are prepared to perform them before using
`apt{nbsp}upgrade`.

. If kernel update instructions were included in the output of the
previous step and you had previously split the RAID using
__drop_primary__ (see <<raid.adoc#_recoverable_testing,Recoverable
Testing>> section in the <<raid.adoc#,RAID Notes for FSL 11>>
document):

+
--
.. Shut the system down (as _root_: `*shutdown -h now*`)
.. Turn the _primary_ disk off.
.. Reboot.
--
+
...
Additional disk rotations may be needed to re-sync the disk order with
other systems if the testing has been going on for an extended period.
...
Before answering `*y*` to proceed, check if any "`new`" packages will
be installed. There should only be the new packages associated with a
kernel update, if there is one. There may also be `new` packages
listed for insecure packages that have been removed (see the
<<Removing insecure packages>> section below). Those can removed
afterwards. If there are packages that your don't recognize, you can
answer `*n*` and investigate before proceeding.
