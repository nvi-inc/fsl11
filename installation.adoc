...
This appendix covers procedures for maintaining your system that go
beyond the initial setup.

=== Update IP address, hostname, FQDN, and other network information

This is useful if the computer is physically moved to a different site
or its network information needs to be be updated for a different
reason. This is typically not needed if you use DHCP, which may still
require some of the changes in the
...
expect amount and that no files were lost or changed
size/modification-times.
...
files were lost or changed size/modification-times.
