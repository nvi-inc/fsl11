...
The `Boot` menu may be where this is set.
...
need to boot one time with no hard disk installed to get it to
recognize using the USB installer as a valid override in the
motherboard's _setup_ utility.
...
If you are asked to remove existing software RAID partition, select
`Yes`.
...
SPACE` area.  For more complicated layouts, it may be more expedient,
    ...
    deleting RAID partitions and/or logical volume data, which you must do to continue.  Then you
...
if prompted to write the changes to disk or to configure LVM.
...
This step can easily be performed when the installation stops for
input in the next step, <<Remove installation media>>.
...
Press kbd:[Enter] to proceed after the Debian/kernel version warning.

#TODO: Eliminate need to use kbd:[Enter], by eliminating warning.#
...
shows no recovery in progress. If there is none, shut down the machine
safely. If you installed with a second disk (_secondary_), remove it
