...
* 1.2.1 -- Explain recovery for simple errors from _refresh_secondary_
during a disk rotation; emphasize that in the incidental uses of
_rotation_shutdown_, the `-p` option inhibits a shutdown
...
down _only_ if the RAID is synced. Click the "`Details`" toggle below
for how to handle the script refusing to shutdown.
...
* If the FS is running: you should terminate it, when convenient,
before trying again.
...
progress meter; the `-p` option also inhibits actually shutting down).

* If the RAID is `degraded`: seek expert advice.

====

. After the shutdown has completed, take the disk from the _primary_
slot and put it on the _shelf_.
...
. When the script says it is waiting for the _secondary_ disk to be
loaded:
...
. If the script rejects the disk (it will stop with an error),
corrective action will be needed. Click the "`Details`" toggle below
for how to proceed.
...
IMPORTANT: You should _not_ run _blank_secondary_, as is suggested by
some of the error messages, without expert advice. Running
_blank_secondary_ is only suggested as a reminder for experts.

If an error stop occurred during _refresh_secondary_, you should
generally seek expert advice; be sure to copy down the full error
message to aid troubleshooting. Neither the _secondary_ nor the
_primary_ disk have been harmed.

While generally expert help is recommended, the following three errors
usually have a simple remedy:

[disc]
* The disk is too recent (you probably inserted the _new_ _shelf_
disk).

* `Foreign RAID(s)` were detected (you inserted a disk from a
different computer/RAID).

* The `MD_UUID` doesn't match (also a disk from a different
computer/RAID).

For these errors, the recovery method is basically to try again with
the _correct_ disk. The following steps should resolve the problem:

TIP: If you are unsure, you can always seek expert advice first.

.. Shut the system down (as _root_: `*shutdown -h now*`).

.. After the shutdown has completed, remove the _secondary_ disk and
put it where it belongs, properly labeled.

.. Locate the _correct_ disk.
...
* Insert the _correct_ disk in the _secondary_ slot and turn the slot
on.

.. If this disk is also rejected, you must seek expert advice; be sure
to copy down the full error message to aid troubleshooting. Neither
the _secondary_ nor the _primary_ disk have been harmed.

.. If this disk is accepted, proceed to the next step.
...
`*rotation_shutdown{nbsp}-p*` for a progress meter; the `-p` option
also inhibits actually shutting down). The system can be used for
