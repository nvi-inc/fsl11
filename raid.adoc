...
during a disk rotation; emphasize in incidental uses of the `-p` option of
_rotation_shutdown_ that a shutdown is not included
...
progress meter; this _not_ will include a shutdown).
...
If an error stop occurred during _refresh_secondary_, one of these
two, bullet point, cases applies:

[disc]
* The error was one of:

+
--

.. The disk is too recent (you probably inserted the
_new_ shelf disk).
...
For these errors, click the "`Details`" toggle below
...
... Remove the _secondary_ disk. (Neither this disk nor the _primary_
disk were harmed.)
...
Insert the _correct_ disk in`the _secondary_ slot and turn the
slot on.

... If this disk is also rejected, seek expert advice. Otherwise
proceed to the next step, <<fini,Finish>>.

=====

* For _any_ other error, seek expert advice.

+

Be sure to record the full error message to aid` troubleshooting.

====

. [[fini]]<<fini,Finish>>: If the disk is accepted, let the refresh
run to completion.

+

You can check its progress with _mdstat_ (or use
`*rotation_shutdown{nbsp}-p*` for a progress meter; this will _not_
include a shutdown). The system can be used for operations while the
refresh is in progress, but may be a little slow.
