...
AUID user from needing to enter _sudo_.

This works for _root_ users because _/usr/local/sbin_ appears before
_/usr/local/bin_ in users' `PATH` variables. It works for non-_root_
users because the versions in _/usr/local/sbin_ are only executable by
_root_.
