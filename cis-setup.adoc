...
Version 1.1.6 - September 2024

:sectnums!:
== Document revision history

* 1.1.6 -- Correct host alias setup for _refresh_spare_usr2_ and
_backup_usr2_, add link for updating host alias in subsequent AUID
accounts
...
. [[bashrc_alias]]<<bashrc_alias,Use the new alias in .bashrc>>: Set
user prompts and _xterm_ titles for _oper_, _prog_, and all AUID
accounts. In each `.bashrc` file to be changed:
...
. If you have installed _refresh_spare_usr2_ and have updated the
_operational_ system alias then on _spare_ system:
...
times, it will end with a line like: `Connection to _operational_
closed.`.  Still, the task of recording the host key will have been
accomplished.

. If you have installed _backup_usr2_ and have updated the
_spare_ system alias then on _operational_ system:
...
by switching to each AUID account (`*su{nbsp}&#8209;{nbsp}__AUID__*`,
where `*_AUID_*` is the account name) in turn, then using
`*ssh{nbsp}__spare__*`, where `*_spare_*` is the new alias of the
_spare_ system. The command will ask you to confirm the fingerprint
for the new hostname. After accepting it, you will be logged in to the
_spare_ system, you can then `*exit*` from that session.
