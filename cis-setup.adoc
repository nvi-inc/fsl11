...
* 1.2.4 -- Correct typo in <<root_key,Copy root ssh
key for running backup_usr2>>

* 1.2.3 -- Set `PATH` before restarting _fvwm_ on console

* 1.2.2 -- Add how to allow _operators_ to use _reboot_ and _shutdown_

* 1.2.1 -- Simplify code block for copying _root_ public key to AUID
account
...
To allow _operators_ to use _shutdown_ and _reboot_ add
(respectively):
...
A password will be required. Trailing options can be used with the
...
would be used since there would not be _sudo_ user customization,
Xresources would not be set, and the window manager would not be
replaced.)

+

CAUTION: The `PATH` and other environment variables should be set
before the window manager, _fvwm_, is restarted. This helps make sure
the window manager, and any windows it launches, have the same
environment as the login shell. It is recommended that additional
`PATH` updates be included at the end of the `PATH` stanza in the
original _.profile_ file.
