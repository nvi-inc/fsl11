...
To allow _operators_ to use _rotation_shutdown_ (with any options) and
_refresh_secondary_ (but only with no options or with the `-h` or `-p`
options individually), add (respectively):
...
(respectively) from their AUID accounts (but not as _oper_ or _prog_):

   sudo rotation_shutdown
   sudo refresh_secondary

NOTE: The AUID account's password will be required unless it was
entered for _sudo_ in the previous 15 minutes.
...
it will install scripts _rotation_shutdown_ and _refresh_secondary_
that incorporate the _sudo_ command, so that users in their AUID
accounts can just use (respectively):
...
If the user is allowed to elevate to _root_, also add:
...
note at the end of step (1) about _root_'s configuration):
...
The five numbered steps above can be executed for _oper_ and  _prog_,
the first three for _root_ with:
...
Currently only _bash_ is supported as a login shell for an AUID
account. _tcsh_ could be used if you insert as the first non-comment
line in the AUID _~/.xsession_ file, the line:
...
system and want it to have a backup of your _operational_ system
_/usr2_ partition to it. All steps for installation must be performed
as _root_ on the specified system. You should read all of each step or
sub-step before following it.
...
password (which will make it fail, see the *IMPORTANT* block below)
and with passing environment variables. Use _visudo_ to add:
...
Before _refresh_spare_usr2_ is run, this line will need to be changed
(temporarily) to:
...
After the _refersh_spare_usr2_ is running this line needs to be
changed back to preserve security. The `NOPASSWD:` should only be
present for a short as time as possible. It is only needed for the
start of the connection from the _spare_ computer.
