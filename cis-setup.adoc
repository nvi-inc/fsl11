...
To allow _operators_ to use _rotation_shutdown_ and
 _refresh_secondary_ add (respectively):
...
that incorporate the _sudo_ command, so that an AUID user (only) can
just use (respectively):
