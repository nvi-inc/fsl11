...
The subsection applies a second round of scripted remediations that go
beyond the CIS benchmark. Before applying these remediations, an
account must be created that will have the ability to promote to
_root_. Please see the
...
The AUID account's password will be required. Trailing options can be
used with the commands, as appropriate.
...
possible to support _tcsh_. That would require adding equivalent
promotion machinery that is used in the current _~/.profile_ files for
these accounts be added into the corresponding _~/.login_ files.
